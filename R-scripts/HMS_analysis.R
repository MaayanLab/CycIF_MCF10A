...
##NOTE: Remove 'DrugName+Conc' from first line of file
...
##PCA testing
pcadata <- HMS_Dataset_cytosolic[HMS_Dataset_cytosolic$DrugName=="Neratinib",4:24]
pcalabels <- HMS_Dataset_cytosolic[,1]
pcaresults <- prcomp(pcadata, center = TRUE, scale. = TRUE)
#plot(pcaresults$x[,1:2],col=factor(pcalabels))
plot(pcaresults$x[,1:2])
biplot(prcomp(pcadata, scale. = TRUE))

PCAmethods <- pca(pcadata,scale = "uv", center = T, nPcs = 2, method = "svd")
slplot(PCAmethods, scoresLoadings = c(T,T))#, scol = factor(pcalabels))
str(PCAmethods) # slots are marked with @
PCAmethods@R2

##Batch effect
biplot(x=pcaresults$x[,1:2],y=colnames(pcadata))
...
# 
# library(gPCA)
# temp <- HMS_Dataset_cytosolic[,3:24]
# temp[,1] <- strsplit(temp[,1],"[+]")[[1]][3]
# batch <- temp[,1]
# data <- temp[,2]
